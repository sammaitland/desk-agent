...
question in natural language — *why did this order fill badly?*, *where is
alpha coming from?*, *what went wrong last week?* — and it investigates by
calling a small set of deterministic, tested tools, then explains what it
found with every figure traced to its source.

The architectural rule: **the model orchestrates; the code computes.** Every
number in every answer comes from a Python function the model called, never
from the model itself. Retrieval over design documentation answers *why* the
system is built the way it is; a cost-aware router predicts each query's cost
from past traces. It runs through a CLI, a Slack bot, an MCP server and a
Streamlit dashboard, all over the same tool layer.
...
**The demonstration data is synthetic.** The blotter is generated by a seeded
simulator that reproduces the trading system's real mechanics — Co1/Co2 with
L/U tails, bucket-driven leg weights, index-relative alpha, ticker-level order
aggregation, stop-loss orphans — and conforms to the live system's event
schema, so real paper-account output populates the same tables. Five
operational failures are planted deterministically so the agent has real
things to investigate. Nothing here is a claim about the strategy's
performance.
