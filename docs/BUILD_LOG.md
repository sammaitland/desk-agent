...
The desk agent was built in phases, each solving one problem and each
teaching something the previous one had assumed. This is the record, in the
order it happened. `README.md` is the summary; this is the detail.

Read it for the reasoning rather than the instructions — several sections
describe decisions that were later revised, and where that happened the
revision is recorded rather than the original quietly edited.
...
## Open questions — since resolved

1. **Longlist/Shortlist ordering.** The event schema had primary filters →
   Shortlist and secondary signals → Longlist; the trading repo's
   `ARCHITECTURE.md` stated the reverse. The schema was right, and the trading
   repo was corrected. Longlist is a manual-inspection export, not a pipeline
   data contract.
2. **Index count.** The event schema's enum includes VOX; the trading repo
   names five ETFs. Both are right: VOX constituents are folded into VGT by
   clustering, so VOX appears in historical tags but is not a live universe.

## Phase 1 — Tool layer

Seven deterministic functions in `src/tools/` at this stage; an eighth
(`search_documentation`) arrives in Phase 8, and `query_blotter` later gains a
`candidates` entity for the screening funnel. The agent will call these; it
